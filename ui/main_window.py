...
# QAction / QIODeviceBase 位置兼容：PySide6 在 QtGui/QIODeviceBase，PySide2 在 QtWidgets/QIODevice
try:
...
class MaterialLibraryWindow(QtWidgets.QMainWindow):
    ...
    def _load_data(self):
        ...
        # 启动后延迟检测重复 UUID
        qt_single_shot(500, self._check_duplicate_uuids)
    ...
    def _refresh_keep_current(self, reload_materials=False):
        ...
        if reload_materials:
            qt_single_shot(300, self._check_duplicate_uuids)
    ...
    def _setup_ui(self):
        ...
        # 拖拽分割手柄时强制预览保持方形
        qt_connect(main_splitter.splitterMoved, self._on_main_splitter_moved)
    ...
    def _create_toolbar(self):
        ...
        qt_connect(self._ai_tools_menu.aboutToShow, self._refresh_ai_tools_menu)
        ...
        qt_connect(self._quick_tools_menu.aboutToShow, self._refresh_quick_tools_menu)
        ...
        qt_connect(help_btn.clicked, self._on_help)
    ...
    def _create_left_panel(self):
        ...
        qt_connect(self._lib_combo.currentIndexChanged, self._on_library_switched)
        ...
        qt_connect(refresh_proj_btn.clicked, self._on_refresh_project)
        ...
        qt_connect(self._proj_category_tree.categorySelected, self._on_proj_category_selected)
        qt_connect(self._proj_category_tree.categoriesMultiSelected, self._on_proj_categories_multi_selected)
        qt_connect(self._proj_category_tree.categoryAdded, self._on_proj_category_added)
        qt_connect(self._proj_category_tree.categoryEdited, self._on_proj_category_edited)
        qt_connect(self._proj_category_tree.categoryDeleted, self._on_proj_category_deleted)
        qt_connect(self._proj_category_tree.openFolderRequested, self._on_proj_open_folder)
        qt_connect(self._proj_category_tree.topLevelCategoryAdded, self._on_proj_add_top_level_category)
    ...
    def _create_connections(self):
        ...
        qt_connect(self._thumbnail_grid.openFolderRequested, self._on_open_material_folder)
        qt_connect(self._thumbnail_grid.moveRequested, self._on_move_material)

        qt_connect(self._favorites_panel.collectionSelected, self._on_fav_collection_selected)
        qt_connect(self._favorites_panel.collectionAdded, self._on_fav_collection_added)
        qt_connect(self._favorites_panel.collectionRenamed, self._on_fav_collection_added)
        qt_connect(self._favorites_panel.materialDropOnCollection, self._on_fav_drop_material)
        qt_connect(self._favorites_panel.collectionDeleted, self._on_fav_collection_deleted)

        qt_connect(self._thumbnail_grid.materialSelected, self._on_material_selected)
        qt_connect(self._thumbnail_grid.materialApplied, self._on_material_applied)
        qt_connect(self._thumbnail_grid.favoriteToggled, self._on_favorite_toggled)
        qt_connect(self._thumbnail_grid.addToFavoriteRequested, self._on_add_to_favorite)
        qt_connect(self._thumbnail_grid.editMaterialRequested, self._show_edit_dialog)
        qt_connect(self._thumbnail_grid.copyToProjectRequested, self._on_copy_to_project)
        qt_connect(self._thumbnail_grid.pasteRequested, self._on_paste)
        ...
        qt_connect(self._thumbnail_grid.variantVersionDeleteRequested, self._on_variant_version_delete)
        qt_connect(self._thumbnail_grid.variantLodDeleteRequested, self._on_variant_lod_delete)
        qt_connect(self._thumbnail_grid.createMaterialRequested, self._on_create_material)
        qt_connect(self._thumbnail_grid.createDomeLightRequested, self._on_create_dome_light)
        qt_connect(self._thumbnail_grid.assignHdrToDomeRequested, self._on_assign_hdr_to_dome)
        qt_connect(self._thumbnail_grid.importSingleTextureRequested, self._on_import_single_texture)
        qt_connect(self._thumbnail_grid.importTexturesSharedUVRequested, self._on_import_textures_shared_uv)
        qt_connect(self._thumbnail_grid.assignTextureToMaterialRequested, self._on_assign_texture_to_material)
        qt_connect(self._thumbnail_grid.aiAnalysisRequested, self._on_ai_analysis)

        self._clipboard = []  # 复制的材质 ID 列表
        self._asset_overlay = None  # 资产创建截图窗口（跨批次复用）
        self._capture_overlay = None  # 预览面板截图窗口（单例）
        qt_connect(self._right_panel.editRequested, self._on_edit_material)
        qt_connect(self._right_panel.favoriteToggled, self._on_favorite_toggled)
        qt_connect(self._thumbnail_grid.exportPresetRequested, self._on_grid_export_preset)
        self._thumbnail_grid.createAssetRequested.connect(
            lambda _: self._on_create_asset())
        qt_connect(self._thumbnail_grid.deleteRequested, self._on_grid_delete)
        ...
        qt_connect(self._thumbnail_grid.dragDroppedOnViewport, self._on_drag_dropped)
        qt_connect(self._thumbnail_grid.previewNodeRequested, self._on_preview_node)
        qt_connect(self._thumbnail_grid.importZlightAsRenderer, self._on_import_zlight_as_renderer)
        qt_connect(self._thumbnail_grid.applyLightToSelectionRequested, self._on_apply_light_to_selection)

        qt_connect(self._search_bar.searchChanged, self._on_search)
        ...
        qt_connect(self._thumbnail_grid.columnCountChanged, self._on_grid_thumb_changed)
        qt_connect(self._sort_combo.currentIndexChanged, self._on_sort_changed)
        qt_connect(self._panel_toggle_btn.clicked, self._on_panel_toggle)

        qt_connect(self._thumbnail_grid.selectionChanged, self._update_status_bar)
        qt_connect(self._thumbnail_grid.selectionChanged, self._update_batch_action_bar)

        # 批量操作栏信号
        qt_connect(self._batch_action_bar.renameRequested, self._on_batch_rename)
        qt_connect(self._batch_action_bar.tagRequested, self._on_batch_tag)
        qt_connect(self._batch_action_bar.moveRequested, self._on_batch_move)
        qt_connect(self._batch_action_bar.copyRequested, self._on_batch_copy)
        qt_connect(self._batch_action_bar.deleteRequested, self._on_batch_delete)
        qt_connect(self._batch_action_bar.clearSelectionRequested, self._on_batch_clear_selection)
    ...
    def _on_search_tag_cleared(self):
        """搜索栏标签筛选清除"""
        # 重入保护：pyd（Cython 编译）下该槽可能被信号链路循环触发，直接返回打断
        if getattr(self, "_tag_clearing_in_progress", False):
            return
        self._tag_clearing_in_progress = True
        try:
            if self._use_mock:
            ...
            self._tag_clearing_in_progress = False
    ...
    def _on_categories_multi_selected(self, cat_ids, all_desc_ids, root_lib="materials"):
        ...
        dicts = [m.to_dict(include_thumb=False) for m in filtered]
        ...
        if os.path.isfile(config_path):
            try:
                ...
                if not parent_id:
                    ...
                    # 删除子分类：从 default_sub_categories 中移除
                    if "default_sub_categories" in cfg and root_lib in cfg["default_sub_categories"]:
                        cfg["default_sub_categories"][root_lib] = [
                            c for c in cfg["default_sub_categories"][root_lib]
                            if c[0] != cat_id
                        ]

                with open(config_path, "w", encoding="utf-8") as f:
                    json.dump(cfg, f, indent=4, ensure_ascii=False)
                print(f"[MaterialLibrary] 从 config.json 移除分类: {cat_id}")
            except Exception as e:
    ...
    def _on_material_dropped_on_category(self, mat_id, cat_id, root_lib=""):
        ...
        if not hasattr(self, '_cat_refresh_timer'):
            ...
            qt_connect(self._cat_refresh_timer.timeout, self._batch_drop_refresh)
    ...
    def _ensure_material_thumb(self, material):
        """确保 material dict 带缩略图字节（网格批量填充用 include_thumb=False 后按需补读）"""
        if not material or material.get("thumb_bytes"):
            return
        if not material.get("is_zasset"):
            return
    ...
    def _show_edit_dialog(self, mat_dict):
        ...
        apply_font_size_to_widget(dlg, fs)
        if qt_exec(dlg) != QtWidgets.QDialog.DialogCode.Accepted: return
    ...
    def _on_fav_collection_selected(self, coll_id, material_ids):
        ...
        if self._use_mock:
            ...
            self._thumbnail_grid.set_materials([m.to_dict(include_thumb=False) for m in favs])
    ...
    def _on_batch_rename(self, materials):
        ...
        if qt_exec(dlg) == QtWidgets.QDialog.DialogCode.Accepted:
    ...
    def _get_maya_parent(self):
        ...
        try:
            ...
            _, _, _, wrapInstance, _ = get_qt_modules()
    ...
    def _on_ai_analysis_batch(self, config=None):
        ...
        for i, mat in enumerate(selected):
            ...
            self._ensure_material_thumb(mat)
        ...
        if review:
            ...
            qt_connect(dlg.batchApplied, self._on_batch_ai_applied)
            qt_exec(dlg)
        else:
    ...
    def _on_create_asset(self):
        ...
        dlg.setModal(False)
        qt_connect(dlg.exportConfigReady, self._on_export_config_ready)
        qt_connect(dlg.finished, self._on_export_dialog_closed)
        dlg.show()
        self._export_dialog = dlg
    ...
    def _on_export_config_ready(self, base_config):
        ...
        if is_real_batch:
            ...
            qt_connect(self._batch_progress.cancelled, self._on_batch_cancelled)
            qt_connect(self._batch_progress.skip_current, self._on_batch_skip_current)
        else:
    ...
    def _on_settings(self):
        ...
        qt_connect(dialog.settingsChanged, self._on_settings_changed)
