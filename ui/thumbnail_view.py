...
def _load_context_menu_preset() -> dict:
    """从 context_menu_preset.json 加载右键菜单预设"""
    import os, json
...
class ThumbnailGridWidget(QtWidgets.QStackedWidget):
    ...
    def _on_table_double_clicked(self, index):
        ...
        # 根据双击命令预设执行
        sub_lib = mat.get('sub_library', '')
        cmd, option = self._get_dc_cmd_and_option(sub_lib)
        if cmd == 'none' or not cmd:
            return

        print(f"[MaterialLibrary] \u53cc\u51fb\u6267\u884c: {cmd} / {option} - {mat.get('name_cn')}")

        self._execute_dc_command(cmd, option, mat, sub_lib)
    ...
    def _on_table_context_menu(self, pos):
        ...
        sub_lib = mat.get('sub_library', '')
        ctx_preset = _load_context_menu_preset()
    ...
    def _get_dc_cmd_and_option(self, sub_lib: str) -> tuple:
        """从双击预设读取命令和子选项，兼容新旧格式"""
        dc_preset = _load_double_click_preset()
        entry = dc_preset.get(sub_lib, "none")
        if isinstance(entry, str):
            return entry, ""
        return entry.get("cmd", "none"), entry.get("option", "")
    ...
    def _execute_dc_command(self, cmd: str, option: str, mat: dict, sub_lib: str):
        """执行双击命令"""
    ...
    def _on_context_menu(self, pos, card):
        ...
        if _is_ctx_enabled(sub_lib, "import", ctx_preset):
            if sub_lib == 'textures':
        ...
        mat_actions = {}
        delete_actions = {}
        if _is_ctx_enabled(sub_lib, "import_geometry", ctx_preset) and variant_types and json_path.endswith('.zasset'):
            from ..core.zasset_io import ZassetIO
        ...
        if sub_lib == 'materials':
            ...
            if _is_ctx_enabled(sub_lib, "create_dome_light", ctx_preset):
                preset_dir = os.path.join(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                    'Assets', 'HDR_ligt')
                if os.path.isdir(preset_dir):
